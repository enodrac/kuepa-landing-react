...
const Animator = (props) => {
  ...
  const onDragStart = function (event) {
    handleUpdateAnimation({ data: { dragginElement: event?.target?.classList[0] } })
  };
    ...
    if (animation?.dragginElement) {
      ...
      if (element) {
        ...
        if (canvas) {
          ...
          let rectCanvas = document.getElementById('animator_canvas')?.getBoundingClientRect()
  ...
  const handleUpdateAnimation = (params) => {
    setAnimation(prev => ({ ...prev, ...params?.data }))
  }
  ...
  const handleCreateFrame = (params) => {
  ...
  const handleDeleteFrame = (params) => {
  ...
  const handleUpdateElement = (params) => {
    handleUpdateAnimation({
      data: {
        frames: {
          ...(animation?.frames ? animation?.frames : {}),
  ...
  const handleClick = (event) => {
    if (!event?.target?.classList[0]?.includes('canvas_click')) {
      handleUpdateAnimation({ data: { openCanvasOption: false } })
      window.removeEventListener('click', handleClick)
    }
  }

  const handleSideScroll = async (event) => {
              ...
              {animation?.currentElement ?
                <div className={`edit_container`}>
                  ...
                  <Select
                    ...
                    onChange={(option) => handleUpdateElement({ edit: { [option?.label]: option?.value } })}
                  />
                    ...
                    attribute !== 'transform' ?
                      <div className={`edit_element`}>
                        ...
                        <input
                          ...
                          onChange={(e) => handleUpdateElement({ edit: { [e?.target?.name]: e?.target?.value } })}
                        />
                      </div>
                      : null
                  )}
          ...
          >
            {animation?.currentFrame && animation?.frames && animation?.frames[animation?.currentFrame] && Object.keys(animation?.frames[animation?.currentFrame])?.map((element) =>
              <div id={element} className={`${element} ${animation?.currentElement === element ? 'active' : ''}`}
                ...
                onClick={(event) => animation?.incoming ? {} : handleUpdateAnimation({ data: { currentElement: animation?.currentElement === element ? undefined : event?.target?.classList[0] } })}
